...
(defunc f1-t (r1 r2 i n)
  :input-contract (and (natp r1) (natp r2) (natp i) (natp n) (<= i n) (>= i 2) (>= n 2))
...
(defunc zip-list (x y)
  ...
  :output-contract (listp (zip-list x y))
  (if (or (endp x) (endp y))
    nil
    (cons (list (first x)
                (first y))
...
(defunc zip-list-t (x y acc)
  :input-contract (and (listp x) (listp y) (listp acc))
  :output-contract (listp (zip-list-t x y acc))
  (if (or (endp x) (endp y))
    (rev* acc)
...
(defunc zip-list* (x y)
  :input-contract (and (listp x) (listp y))
  :output-contract (listp (zip-list* x y))
  (zip-list-t x y nil))#|ACL2s-ToDo-Line|#
