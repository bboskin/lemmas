...
(suggest-lemma (or a b)
	      :with nand
	      :hyps (booleanp a) (booleanp b))

(thm (IMPLIES (AND (BOOLEANP A) (BOOLEANP B))
         (EQUAL (OR A B)
...
; expressions.
(defdata2 bex-not (list 'not bex))
(defdata2 bex-and (list 'and bex bex))
(defdata2 bex-or (list 'or bex bex))
...
(thm (IMPLIES (AND (VARP BE) (LOVARP LOS))
...
(thm (IMPLIES (AND (LOVARP LOS) (CEXP CE))
...
(suggest-lemma (bex-evaluate be los)
	       :required-expressions (compiler be)
	       :with cex-evaluate)

(thm (IMPLIES (AND (BEXP BE) (LOVARP LOS))
